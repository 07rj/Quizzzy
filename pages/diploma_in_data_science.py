...
if st.session_state.subject and st.session_state.quiz is None:
    ...
    st.subheader(f"Subject: {subjects_map[subject]}")
    ...
    for i in range(1, 100):
        ...
        quiz = f"quiz_{i}"
        module_path = f"questions.{level}.{subject}.{quiz}"
...
if st.session_state.subject and st.session_state.quiz:
    ...
    run_exam(
        questions,
        f"{subjects_map[st.session_state.subject]} - {st.session_state.quiz}",  # FIXED TYPO
        duration=1800
    )